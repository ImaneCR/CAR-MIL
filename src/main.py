...
def one_fold(args,k,ckc_metric,train_p, train_l, test_p, test_l,val_p,val_l):
    ...
    if args.model == 'attmil':
        ...
        model = attmil.AttentionGated(input_dim=args.input_dim,n_classes=args.n_classes,dropout=args.dropout).to(device)
    elif args.model == 'inst_gattmil':
        model = attmil.Inst_AttentionGated(input_dim=args.input_dim,n_classes=args.n_classes,dropout=args.dropout).to(device)
    elif args.model == 'add_gattmil':
        model = attmil.Add_AttentionGated(input_dim=args.input_dim,n_classes=args.n_classes,dropout=args.dropout).to(device)
    elif args.model == 'ibmil':

        _confounder_path = [args.confounder_path]
        model = attmil.IBMIL_AttentionGated(input_dim=args.input_dim,n_classes=args.n_classes,dropout=args.dropout,confounder_path=_confounder_path).to(device)
        init_ckpt = torch.load(args.init_ibmil_ckpt)
        info = model.load_state_dict(init_ckpt['model'] , strict= False)
        print('Loading init ckp ', info)
...
def train_loop(args,model,loader,optimizer,device,amp_autocast,criterion,loss_scaler,scheduler,k,epoch):
    ...
    # Synchronizing before to be safe
...
if __name__ == '__main__':
    ...
    parser.add_argument('--confounder_path', type=str, help='ibmil_con_path')
    parser.add_argument('--init_ibmil_ckpt', type=str, help='ibmil_init')
