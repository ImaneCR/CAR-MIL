...
def five_scores_multi(bag_labels, bag_predictions, sub_typing=False):

    auc_value = roc_auc_score(
        bag_labels, bag_predictions, multi_class='ovr', average='macro'
    )

    preds_bin = np.argmax(bag_predictions, axis=1)
    precision, recall, fscore, _ = precision_recall_fscore_support(
        bag_labels, preds_bin, average='macro', zero_division=0
    )
    #accuracy = accuracy_score(bag_labels, preds_bin)
    accuracy = balanced_accuracy_score(bag_labels, preds_bin)
