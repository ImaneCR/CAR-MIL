...
import argparse, os
from modules import attmil_cf, dsmil, transmil
...
class CustomLoss_COS(nn.Module):
    def __init__(self, loss_type='bce', alpha_att=0.0, alpha_effect=0.0):
        super(CustomLoss_COS, self).__init__()
    ...
    def forward(self, preds, targets, preds_cf=None, 
                bag_attention=None, bag_attention_cf=None):
        ...
        # --- Effect regularization (difference between factual and CF preds) ---
        if preds_cf is not None:
            preds_effect = preds - preds_cf
            loss_reg = self.criterion(preds_effect, targets)

        # --- Attention alignment using cosine distance ---
        if bag_attention is not None and bag_attention_cf is not None:
            # Normalize both to unit vectors along patch dimension
            att_base = F.normalize(bag_attention, p=2, dim=-1)
            att_cf = F.normalize(bag_attention_cf, p=2, dim=-1)
            #Cosine distance (1 - cosine similarity)
            loss_attention = 1.0 - (att_base * att_cf).sum(dim=-1).mean()  

        # --- Combine ---
        total_loss = (
            loss_main
            + self.alpha_effect * loss_reg  
            + self.alpha_att * loss_attention
        )
        return total_loss
...
class CustomLoss_L1(nn.Module):
    ...
    def forward(self, preds, targets, preds_cf=None, bag_attention=None, bag_attention_cf=None):
        ...
        if bag_attention is not None and bag_attention_cf is not None:
            loss_attention = F.l1_loss(bag_attention, bag_attention_cf)
    

        total_loss = loss_main + self.alpha_effect * loss_reg + self.alpha_att * loss_attention
        return total_loss
...
def main(args):
    ...
    for k in range(args.fold_start, args.cv_fold)[args.task_id:args.task_id+1]: # from 0 to 4
        ...
        metrics_df.to_csv(os.path.join(dir_eval, 'cv_metrics_fold_{fold}.csv').format(fold=k))
...
def one_fold(args,k,ckc_metric,train_p, train_l, test_p, test_l,val_p,val_l):
    ...
    if args.model == 'attmil':
        model = attmil_cf.DAttention_CF(input_dim=args.input_dim,n_classes=args.n_classes,dropout=args.dropout,act=args.act).to(device)
    elif args.model == 'gattmil':
        model = attmil_cf.AttentionGated_CF(input_dim=args.input_dim,n_classes=args.n_classes,dropout=args.dropout).to(device)
    
    elif args.model == 'transmil':
        model = transmil.TransMIL_CF(input_dim=args.input_dim,n_classes=args.n_classes,dropout=args.dropout).to(device)

    elif args.model == 'dsmil':
        model = dsmil.MILNet_CF(input_dim=args.input_dim,n_classes=args.n_classes,dropout=args.dropout,act=args.act).to(device)
        args.cls_alpha = 0.5
        args.aux_alpha = 0.5
    ...
    if args.reg_dist == 'COS':
        criterion = CustomLoss_COS(args.loss, args.alpha_att, args.alpha_effect)
    else:
        criterion = CustomLoss_L1(args.loss, args.alpha_att, args.alpha_effect)
...
def train_loop(args,model,loader,optimizer,device,amp_autocast,criterion,loss_scaler,scheduler,k,epoch):
    ...
    end_cuda.record()
    torch.cuda.synchronize()
    time_cuda = start_cuda.elapsed_time(end_cuda)
    print("Elapsed time is: ", time_cuda)
...
if __name__ == '__main__':
    ...
    # CF
    parser.add_argument('--alpha_att', default=0.2, type=float, help='Classification Loss [ce, bce]')
    parser.add_argument('--alpha_effect', default=0.2, type=float, help='Classification Loss [ce, bce]')
    parser.add_argument('--reg_dist',default='L1',type=str, help='attention loss distance [L1,COS]')
    ...
    dir_eval = os.path.join(args.model_path, 'Eval_Flapping')
