...
class MHIM(nn.Module):
    def __init__(self, mlp_dim=512,mask_ratio=0,n_classes=2,temp_t=1.,temp_s=1.,dropout=0.25,act='relu',select_mask=True,select_inv=False,msa_fusion='vote',mask_ratio_h=0.,mrh_sche=None,mask_ratio_hr=0.,mask_ratio_l=0.,da_act='relu',baseline='attn',head=8,attn_layer=0):
        ...
        self.patch_to_emb = [nn.Linear(1024, 512)] # correct later with input dim
    ...
    def forward_eval(self,x):
        ...
        if self.baseline == 'dsmil':
            ...
            x, attn = self.online_encoder(x,return_attn=True,no_norm=True)
